                ...
                <div class="row">
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <select
                        name="board"
                        :class="{'form-control': true, 'error': errors.has('board')}"
                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <input
                        ...
                        name="obtained"
                        min="0"
                        v-model="student.XII_obtained_marks"
                        v-validate="'required'"
                        class="form-control"
                      />
                    </div>
                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <input
                        type="number"
                        name="max"
                        v-model="student.XII_max_marks"
                        class="form-control"
                        v-validate="'required'"
                        v-on:input="percentCalculate1()"
                      />
                      <span
                        id="XII_percent"
                        class="small text-muted pull-right"
                      >Percent = {{ XII_percent }}%</span>
                    </div>
                  </div>
                </div>
                <div class="row">
                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <input
                        type="text"
                        name="institute_name"
                        v-model="student.diploma_institute"
                        class="form-control"
                      />
                    </div>
                  </div>
                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <input
                        type="number"
                        name="max"
                        v-model="student.diploma_max_marks"
                        class="form-control"
                        v-validate="'required'"
                        v-on:input="percentCalculate2()"
                      />
                      <span
                        id="diploma_percent"
                        class="small text-muted pull-right"
                      >Percent = {{ diploma_percent }}%</span>
                    </div>
                  </div>
                </div>
                <div class="row">
...
export default {
  ...
  beforeCreate() {
    axios
      ...
      .then(response => {
        let batch = response.data.data[0].batch;
        this.year_joining = batch.split("-")[0];
      })
      .catch(error => {
        console.log(error);
      });
    axios
      .get("/api/dashboard/student/me/" + this.$parent.username + "/edit")
      .then(response => {
    ...
    axios
      .get("/api/dashboard/student/pe/" + this.$parent.username + "/edit")
      .then(response => {
  ...
  methods: {
    selection() {
      ...
      } else if (this.student.education == "BOTH") {
        $("#twelth").show();
        $("#Diploma").show();
        $("#twelth :input").attr("disabled", false);
        $("#Diploma :input").attr("disabled", false);
    ...
    percentCalculate1() {
      if (
        this.student.XII_obtained_marks != undefined &&
        this.student.XII_max_marks != undefined
      ) {
        this.XII_percent = parseFloat(
          (this.student.XII_obtained_marks / this.student.XII_max_marks) * 100
        ).toFixed(3);
      } else {
        this.XII_percent = 0;
      }
    ...
    percentCalculate2() {
      if (
        this.student.diploma_obtained_marks != undefined &&
        this.student.diploma_max_marks != undefined
      ) {
        this.diploma_percent = parseFloat(
          (this.student.diploma_obtained_marks /
            this.student.diploma_max_marks) *
            100
        ).toFixed(3);
      } else {
    ...
    validateYear() {
      ...
      if (
        ($("#twelth").is(":visible") &&
          this.student.XII_year - this.year < 2) ||
        (this.student.diploma_year - this.year < 3 &&
          $("#Diploma").is(":visible"))
      ) {
        $(".GE").html("Year of passing cannot be less than minimum value");
        $(".GE").focus();
        return false;
      }
      if (
        ($("#twelth").is(":visible") &&
          this.student.XII_year > this.year_joining) ||
        ($("#Diploma").is(":visible") &&
          this.student.diploma_year > this.year_joining)
      ) {
        $(".GE").html("Year of passing cannot be more than admission year");
        $(".GE").focus();
        return false;
      }
      return true;
    ...
    gap_calculator() {
      if (this.validateYear()) {
        this.student.year_gap = 0;
        if (this.student.education == "XII") {
          this.student.year_gap +=
            this.student.XII_year - this.year > 2
              ? this.student.XII_year - this.year - 2
              : 0;
          this.student.year_gap +=
            this.year_joining - this.student.XII_year > 0
              ? this.year_joining - this.student.XII_year
              : 0;
        } else if (this.student.education == "DIPLOMA") {
          this.student.year_gap +=
            this.student.diploma_year - this.year > 3
              ...
              : 0;
          this.student.year_gap +=
            this.year_joining - this.student.diploma_year > 0
              ? this.year_joining - this.student.diploma_year
              : 0;
        } else {
          this.student.year_gap +=
            this.student.XII_year - this.year > 2
              ...
              : 0;
          this.student.year_gap +=
            this.student.diploma_year - this.student.XII_year > 3
              ? this.student.diploma_year - this.student.XII_year - 3
              : 0;
          this.student.year_gap +=
            this.year_joining - this.student.diploma_year > 0
        ...
        this.student.year_gap = "Invalid year";
      }
    },
    getAuthUser(name) {
    ...
    submit() {
      ...
      if (this.student.XII_institute != null)
        this.student.XII_institute = this.$parent.titleCase(this.student.XII_institute);
      if (this.student.diploma_institute != null)
        this.student.diploma_institute = this.$parent.titleCase(this.student.diploma_institute);
      ...
      axios
        .post(storeStudentPeURL + this.student.id, formData, {
          ...
          axios
            ...
            .then(statusresponse => {
              if (statusresponse.status == 200) {
                toastr["success"]("Previous education added!!");
                this.$parent.step = 6;
              }
            })
            .catch(errors => {
